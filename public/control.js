...
const NOTICE_TARGET_LABELS = { command: '지부 공용 단말', workflow: '전자서류 · 체크리스트', archive: '문서 보관소', evidence: '증거물 · 현장사진', cases: '사건철', personnel: '인사기록부', city: '태양시 정보' };
...
async function uploadEvidence(form) {
  ...
  try {
    ...
    app.state = data.state; render(); resetEvidenceForm(form); toast(editing ? '증거물 정보가 수정되었습니다.' : '증거 사진이 비공개로 저장되었습니다.');
...
function controlView(name){name=['operations','cases','personnel','archive','evidence','workflow'].includes(name)?name:'operations';for(const view of ['operations','cases','personnel','archive','evidence','workflow'])$('#control-'+view).hidden=view!==name;document.querySelectorAll('[data-control-view]').forEach(button=>button.classList.toggle('active',button.dataset.controlView===name));history.replaceState(null,'','#'+name);}
